 ...
 * 1.    首先程序到 main 方法这里，使用标准化输出 Son 类中的 factor 类成员变量，
 * 但是 Son 类中并没有定义这个类成员变量。于是往父类去找，
 * 我们在 Father 类中找到了对应的类成员变量，于是触发了 Father 的初始化。
 * 2.     但根据我们上面说到的初始化的 5 种情况中的第 3 种
 * （当初始化一个类的时候，如果发现其父类还没有进行过初始化，则需要先触发其父类的初始化）。
 * 我们需要先初始化 Father 类的父类，也就是先初始化 Grandpa 类再初始化 Father 类。
