...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        if (key == null || value == null) throw new NullPointerException(); //无法使用空值.
        int hash = spread(key.hashCode());  //获取hash值
        int binCount = 0;
        for (Node<K, V>[] tab = table; ; ) { //自旋table
            Node<K, V> f;
            int n, i, fh;
            if (tab == null || (n = tab.length) == 0) //初始化table
                tab = initTable(); //初始化table
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) { //如果table idx为空,插入new Node
                if (casTabAt(tab, i, null,
                        new Node<K, V>(hash, key, value, null)))  //cas方式插入该节点
                    break;                   // no lock when adding to empty bin
            } else if ((fh = f.hash) == MOVED)
                tab = helpTransfer(tab, f);
            else {
                V oldVal = null; //节点上存在值, 对该节点头节点进行加锁
                synchronized (f) {
                    if (tabAt(tab, i) == f) { //
                        if (fh >= 0) {
                            binCount = 1;
